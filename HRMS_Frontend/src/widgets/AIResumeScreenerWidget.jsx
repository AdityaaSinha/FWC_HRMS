...
export default function AIResumeScreenerWidget() {
  ...
  // --- NEW: State for the job description text ---
  ...
  // When a job is selected, auto-fill the textarea
  ...
  const handleScreenResume = async () => {
    ...
    // --- UPDATED: Send the text from the textarea ---
    ...
    try {
      ...
      if (data.error) {
        ...
        throw new Error('AI returned invalid JSON. Could not parse.');
      }
  ...
  return (
    <Card className="flex flex-col">
      ...
      <div className="space-y-4">
        ...
        >
          {isLoading ? <AILoadingSpinner /> : <><Icon as="Zap" size={18} className="mr-2" /> Screen Resume</>}
        </button>
      </div>
      ...
      {/* Result display (no change needed) */}
      {result && (
        <div className="mt-4 p-4 bg-gray-50 dark:bg-gray-700 rounded-lg">
          ...
          <div className="mt-3">
            <span className="text-xs font-semibold uppercase text-gray-500 dark:text-gray-400">Key Strengths:</span>
            <div className="flex flex-wrap gap-1 mt-1">
              {result.keyStrengths.map((key, i) => (
                <span key={i} className="px-2 py-0.5 bg-green-100 text-green-800 dark:bg-green-900 dark:text-green-200 text-xs rounded-full">{key}</span>
              ))}
            </div>
          </div>
          
          <div className="mt-3">
            <span className="text-xs font-semibold uppercase text-gray-500 dark:text-gray-400">Potential Gaps:</span>
            <div className="flex flex-wrap gap-1 mt-1">
              {result.potentialGaps.map((key, i) => (
                <span key={i} className="px-2 py-0.5 bg-red-100 text-red-800 dark:bg-red-900 dark:text-red-200 text-xs rounded-full">{key}</span>
              ))}
            </div>
          </div>
        </div>
